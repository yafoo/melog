...
class Article extends Model
{
    ...
    async getIndexList(condition, rows=10){
        return await this.db.table('article a').field('a.id,a.cate_id,a.user_id,a.title,a.writer,a.click,a.description,a.add_time,c.cate_name,c.cate_dir').join('cate c', 'a.cate_id=c.id').where(condition).order('a.id', 'desc').limit(rows).cache(this.cacheTime).select();
    }

    // 栏目文章列表及分页
    async getPageList(condition, page_size=10) {
        return await this.db.field('id,cate_id,user_id,title,writer,source,click,keywords,description,add_time').where(condition).order('id', 'desc').cache(this.cacheTime).pagination(this.$pagination.cate, page_size);
    }

    // 搜索文章列表及分页
    async getSearchList(condition, pageSize=10) {
        return await this.db.table('article a').field('a.id,a.cate_id,a.user_id,a.title,a.writer,a.keywords,a.click,a.description,a.add_time,c.cate_name,c.cate_dir').join('cate c', 'a.cate_id=c.id').where(condition).order('a.id', 'desc').cache(this.cacheTime).pagination(undefined, pageSize);
    }
